...
{ // подъём антенны никогда не ухудшает линк; выход над полог заметно помогает
  ...
  // синтетическая проверка физики: на площадке подвес ограничен 5–6 м (лес), но модель
  // обязана реагировать на высоту — иначе она не различает «в пологе» и «над пологом»
  ok('подъём 12→25 м снижает потери (проверка физики, не план)', seq[4] > seq[2] + 0.5,
